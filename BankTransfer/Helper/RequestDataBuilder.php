<?php
namespace Payments\BankTransfer\Helper;
...
class RequestDataBuilder extends \Payments\Core\Helper\AbstractDataBuilder
{
    ...
    public function buildSaleService($store, $merchantId, $quote, $bankCode, $deviceId=null)
    {
        ...
        $storeName = $this->gatewayConfig->getValue(
            "payment/payments_bank_transfer/bank_store_name",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    ...
    public function buildRefundService($requestId, $order, $paymentMethod, $merchantId)
    {
        $request = new \stdClass();
        $request->merchantID = $merchantId;
        $request->partnerSolutionID = \Payments\BankTransfer\Helper\RequestDataBuilder::PARTNER_SOLUTION_ID;
        $developerId = $this->gatewayConfig->getValue(
            "payment/payments_sa/developer_id",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );

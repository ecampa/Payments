define([
    ...
    'Payments_WeChatPay/js/action/get-qr-data',
    'Payments_WeChatPay/js/action/check-status',
    'Payments_WeChatPay/js/model/loader',
    'jquery',
    'ko',
    'mage/validation',
    'mage/translate'
            ...
            return checkStatusAction(this.orderId, final, cancel).then(
                function (response) {
                    ...
                    if (response.is_settled) {
                        ...
                        return this.complete(this.successUrl);
                    }

                    if (response.is_failed) {
                        loader.stopLoader();
                        this.addMessage(response.status_msg, 'error');
                        return this.complete(this.failureUrl);
                    }

<?php
namespace Payments\Address\Controller\Index;
    ...
    public function execute()
    {
        ...
        $data = [
            ...
            'needForce' => (bool)$this->scopeConfig->getValue(
                "payment/payments_sa/address_force_normal",
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            ),
            'updateFields' => [],
            'needUpdate' => false,
        ...
        if ($this->scopeConfig->getValue(
            ...
            $result = $this->gatewayApi->checkAddress(
                $merchantId,
            ...
            if (empty($result) || $result->reasonCode != 100) {
                ...
                foreach ($fieldsToCheck as $key => $magentoKey) {
                    if (!empty($davReply[$key]) && $davReply[$key] != $shippingAddress[$magentoKey]) {
                        $needUpdate = true;
                        $fieldsUpdate[$magentoKey] = $davReply[$key];
                        if (preg_match('/^street(\d+)/', $magentoKey, $matches)) {
                            $streetIndex = $matches[1] - 1;
                            $data['normalizationData']['street[' . ($streetIndex) . ']'] = $davReply[$key];
                            ...
                            $data['normalizationData'][$magentoKey] = $davReply[$key];
                        }
                    }
                }

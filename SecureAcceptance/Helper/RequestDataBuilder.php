...
use Payments\Core\Model\Source\AuthIndicator;
...
class RequestDataBuilder extends \Payments\Core\Helper\AbstractDataBuilder
{
    ...
    /**
     * @var \Payments\SecureAcceptance\Helper\Vault
     */
    private $vaultHelper;
    ...
    private function buildBasePaymentData(Quote $quote, Quote\Address $billingAddress)
    {
        ...
        $params = [
            ...
            'override_custom_receipt_page' => $this->urlBuilder->getUrl('paymentssa/index/placeorder'),
            'override_custom_cancel_page' => $this->urlBuilder->getUrl('paymentssa/index/cancel'),
            'tax_amount' => $this->formatAmount($shippingAddress->getBaseTaxAmount()),
            'partner_solution_id' => self::PARTNER_SOLUTION_ID
        ];

        if ($this->gatewayConfig->getAuthIndicator() != \Payments\Core\Model\Source\AuthIndicator::UNDEFINED) {
    ...
    public function buildCreateTokenRequest()
    {
        ...
        $data['override_custom_receipt_page'] = $this->urlBuilder->getUrl('paymentssa/index/createtoken');

define(
    ...
    function (
        ...
        return Component.extend({
            defaults: {
                active: false,
                template: 'Payments_SecureAcceptance/payment/form',
                code: 'payments_sa',
                imports: {
            ...
            placeOrderWithSOP: function() {
                ...
                $.when(
                    ...
                    function () {
                        return $.ajax({
                            url: urlBuilder.build("paymentssa/index/loadsilentdata"),
                            type: "post",
                            data: queryString,
                            dataType: 'json',
                            showLoader: true
            ...
            placeOrderWithWebMobile: function () {
                ...
                $.when(
                    ...
                    function () {
                        return $.ajax({
                            url: urlBuilder.build("paymentssa/index/loadinfo"),
                            type: "post",
                            dataType: 'json',
                            data: queryString
                        })
            ...
            getIFrame: function () {
                ...
                var loading = '<div id="payments-modal-content" style="width: 500px; padding: 0;position: fixed;z-index: 9999;margin: 0 auto;left: 0;right: 0; top:0">' +
                    '<div id="payments-modal-modal-child" style="position: relative; background-color: #fff;">' +
                    '<div style="width:100%;text-align:right;">' +
                    '<a href="" id="payments-close-iframe">X</a>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
                if (document.getElementById('payments-view-detail-processing-modal')) {
                    $('#payments-view-detail-processing-modal').remove();
                }
                if (!$('.loading-mask')) {
                ...
                if (!document.getElementById('payments-view-detail-processing-modal')) {
                    ...
                    $(overlay).attr('id', 'payments-view-detail-processing-modal');
                    ...
                    $.when(
                        ...
                        function () {
                            return $.ajax({
                                url: urlBuilder.build("paymentssa/index/loadiframe"),
                                type: "post",
                                dataType: 'json',
                                data: queryString
                            })
                        ...
                        function (data) {
                            ...
                            var modal = document.getElementById("payments-modal-modal-child");
                            iframe.onload = function () {
                                try {
                                    var form = iframe.contentWindow.document.getElementById('payments-iframe-form');
                                    $(form).submit();
                                } catch (err) {
                                }
                            };
                    ...
                    $('#payments-close-iframe').click(function () {
                        $('#payments-view-detail-processing-modal').remove();
                        $('.loading-mask').hide();
                        return false;
                    });
            ...
            updateIframePosition: function () {
                ...
                $("#payments-modal-modal-child").css({top: iframe_height, width: iframe_width});
            },

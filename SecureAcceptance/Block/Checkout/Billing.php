<?php
namespace Payments\SecureAcceptance\Block\Checkout;
...
class Billing extends \Magento\Multishipping\Block\Checkout\Billing
{
    ...
    public function getCustomerTokens()
    {
        ...
        if (!empty($customerPaymentTokens)) {
            ...
            foreach ($customerPaymentTokens as $paymentToken) {
                ...
                if ($paymentToken->getPaymentMethodCode() !== \Payments\Core\Model\Config::CODE) {
                    continue;
                }
    ...
    {
        return $this->_urlBuilder->getUrl('paymentssa/index/createtoken');
    }

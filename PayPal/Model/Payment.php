...
class Payment extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    /**
     * @var \Payments\PayPal\Model\Config
     */
    ...
    /**
     * @var \Payments\PayPal\Gateway\Request\VaultSaleRequestBuilder
     */
     ...
     * @param \Payments\PayPal\Model\Config $gatewayConfig
    ...
    public function __construct(
        ...
        \Payments\PayPal\Helper\RequestDataBuilder $dataBuilder,
        \Magento\Framework\UrlInterface $urlBuilder,
        \Magento\Sales\Model\Order\Payment\Transaction\BuilderInterface $transactionBuilder,
        \Payments\Core\Model\LoggerInterface $loggerModel,
        \Magento\Payment\Gateway\Data\PaymentDataObjectFactoryInterface $paymentDataObjectFactory,
        \Payments\PayPal\Gateway\Request\VaultSaleRequestBuilder $vaultSaleRequestBuilder,
    ...
    public function authorize(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $payment->setAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_RESPONSE_CODE, $response->reasonCode);
        $payment->setAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_AUTH_TXN_ID, $response->requestID);
        $payment->setAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_TXN_ID, $response->requestID);
    ...
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        if ($billingAgreementId = $payment->getAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_BA_ID)) {
            // paypal BA sale
            ...
            if ($payment->getParentTransactionId()) { // doing prior auth capture
                ...
                if (! $requestId = $payment->getAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_AUTH_TXN_ID)) {
    ...
    {
        $payment->setMethod(\Payments\PayPal\Model\Payment::CODE);
    ...
    public function getConfigPaymentAction()
    {
        ...
        if ($infoInstance instanceof \Magento\Payment\Model\InfoInterface
            && $infoInstance->getAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_BA_ID)
        ) {
    ...
    {
        return $this->urlBuilder->getUrl('paymentspaypal/express/start');
    }
    ...
    private function runPaypalSale($payment)
    {
        ...
        $payment->setAdditionalInformation(\Payments\PayPal\Model\Express\Checkout::PAYMENT_INFO_TRANSPORT_ORDER_SETUP_TXN_ID, $orderSetupResponse->requestID);
    ...
    private function handlePaymentStatus($payment, $decision)
    {
        switch ($decision) {
            ...
            case 'ERROR':
            case 'REJECT':
                $payment->setAdditionalInformation(\Payments\PayPal\Model\Info::PAYMENT_STATUS, Order::STATE_CANCELED);
                break;

            case 'REVIEW':
                $payment->setAdditionalInformation(\Payments\PayPal\Model\Info::PAYMENT_STATUS, Order::STATE_PAYMENT_REVIEW);
                $payment->setAdditionalInformation(\Payments\PayPal\Model\Info::IS_FRAUD, true);
                break;
        }

        $payment->setAdditionalInformation(\Payments\PayPal\Model\Info::PAYMENT_STATUS, Order::STATE_PROCESSING);
    }

<?php
namespace Payments\ECheck\Cron;
...
class Status
{
    ...
    /**
     * @var \Payments\ECheck\Gateway\Config\Config
     */
    private $config;
    ...
    */
    ...
    public function execute()
    {
        if (!(bool)(int)$this->scopeConfig->getValue(
            "payment/payments_echeck/active",
        ...
        try {
            ...
            $paymentCollection->addFieldToFilter('main_table.method', 'payments_echeck');
    ...
    private function updateOrder($eventType, $order)
    {
        ...
        if (!in_array($eventType, $this->config->getAcceptEventType())
            ...
            $this->sendEmail($order, $eventType, 'template_echeck_unknown');
            $updateStatus = false;
        }
        $inCounter = 0;
        ...
        if ($updateStatus && $inCounter > 1) {
            ...
            $this->sendEmail($order, $eventType, 'template_echeck_multi');
            $updateStatus = false;
        }
        if ($updateStatus && in_array($eventType, $this->config->getAcceptEventType())) {
    ...
    public function sendEmail($order, $eventType, $templateId = 'template_echeck_unknown')
    {
        $emailTemplateVariables = [];
        $emailTempVariables = ['order' => $order, 'event_type' => $eventType];
        $sender = $this->scopeConfig->getValue(
            "payment/payments_sa/dm_fail_sender",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    ...
    {
        $data = [];
        if (false && $this->scopeConfig->getValue(
            "payment/payments_echeck/is_test",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        )) {
            ...
            foreach ($paymentCollection as $payment) {
                $data[$payment->getLastTransId()] = [
                    ...
                    self::EVENT_TYPE_COLUMN => $this->scopeConfig->getValue(
                        "payment/payments_echeck/test_event_type",
                        \Magento\Store\Model\ScopeInterface::SCOPE_STORE
                    )
                ];
            }
        ...
        $reportUrl = $this->scopeConfig->getValue(
            "payment/payments_echeck/service_url",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        ...
        $username = $this->scopeConfig->getValue(
            "payment/payments_echeck/merchant_username",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $password = $this->scopeConfig->getValue(
            "payment/payments_echeck/merchant_password",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $period = (int)$this->scopeConfig->getValue(
            "payment/payments_echeck/report_check_period",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );

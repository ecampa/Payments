...
window.PaymentsAdminOrderHelper = Class.create();
PaymentsAdminOrderHelper.prototype = {
    initialize : function (methodCode, controller, orderSaveUrl, nativeAction) {
        var prepare = function (event, method) {
            if (method === 'payments_sa') {
                this.preparePayment();
            } else {
                jQuery('#edit_form')
                    .off('submitOrder.payments');
            }
        };
    ...
    preparePayment : function () {
        ...
        jQuery('#edit_form')
            .off('submitOrder')
            .on('submitOrder.payments', this.submitAdminOrder.bind(this));
    },

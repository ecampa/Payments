...
class Dm
{
    ...
    const SECURE_ACCEPTANCE_METHOD = 'payments_sa';
    ...
    */
    ...
    public function sendEmail($order, $storeId)
    {
        ...
        $emailTemplate = $this->scopeConfig->getValue(
            "payment/payments_sa/dm_fail_template",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $storeId
        );
    ...
    public function execute()
    {
        foreach ($this->storeManager->getStores() as $storeId => $store) {
            if (!$this->scopeConfig->getValue(
                "payment/payments_sa/enable_dm_cron",
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                $store->getId()
            )) {
            ...
            $url = $this->scopeConfig->getValue(
                "payment/payments_sa/report_url",
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                $store->getId()
            );
            if ($url === null) {
            ...
            if (preg_match_all(
                ...
                $params['password'] = $this->crypt->decrypt($this->scopeConfig->getValue(
                    "payment/payments_sa/report_password",
                    \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                    $store->getId()
                ));
    ...
    private function processCancel($payment_info, $order, $storeId)
    {
        ...
        if ($payment_info[$order->getId()]) {
            if ($paymentMethod == self::VISA_CHECKOUT_METHOD) {
                ...
                /** @var \Payments\PayPal\Model\Payment $methodInstance */
                $methodInstance = $order->getPayment()->getMethodInstance();
                $methodInstance->cancel($order->getPayment());
            } else {
                $this->gatewayApi->setPayment($payment_info[$order->getId()]['payment']);
    ...
    private function saveAuthorize($params, $payment_info, $order, $isTokenPaid, $storeId)
    {
        ...
        if ($payment_info[$order->getId()]['NewDecision'] == 'ACCEPT') {
            ...
            if ($paymentMethod == self::PAYPAL_METHOD || $paymentMethod == self::VISA_CHECKOUT_METHOD || $paymentMethod == "payments_applepay") {
            ...
            if ($order->getState() != 'pending_payment' && !$isTokenPaid && $paymentMethod == self::SECURE_ACCEPTANCE_METHOD) {
                ...
                $result = $this->gatewayApi->convertToProfile($profile_data, $storeId);
    ...
    private function composeParams($storeId)
    {
        ...
        $params['username'] = $this->scopeConfig->getValue(
            "payment/payments_sa/report_username",
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $storeId
        );
